...
public class XffectComponentCustom : Editor
{
    ...
    void SavePreview()
    {
        ...
        //int mOutputWidth = 256;
        //int mOutputHeight = 256;


        //int left = screenWidth / 2 - mOutputWidth / 2;
        //int top = screenHeight / 2 - mOutputHeight / 2;

        //Rect outRect = new Rect(left, top, mOutputWidth, mOutputHeight);

        //tex2D.ReadPixels(outRect, 0, 0);
        //tex2D.Apply();

        //byte[] bytes = tex2D.EncodeToPNG();
        ////System.IO.File.WriteAllBytes("test.png", bytes);

        //Script.PreviewData = new byte[bytes.Length];

        //System.Buffer.BlockCopy(bytes, 0, Script.PreviewData, 0, bytes.Length);
